...
function GetGoogleAuthClient(config: Igoogleauth_credentials): any {
    ...
    if (config != null) {
        if (typeof config.serviceaccount === "string" && !Util.IsNullEmpty(config.serviceaccount)) {
            config.serviceaccount = JSON.parse(config.serviceaccount);
        }
        if (!Util.IsNullEmpty(config.serviceaccount)) {
            result.auth = new GoogleAuth({
                scopes: config.scopes.split(",").join(" "),
                credentials: config.serviceaccount
            });
        }
        if (!Util.IsNullEmpty(config.clientid) || !Util.IsNullEmpty(config.clientsecret) || !Util.IsNullEmpty(config.redirecturi)) {
            result.Client = new OAuth2Client(
                config.clientid,
                config.clientsecret,
                config.redirecturi
            );
            if (!Util.IsNullEmpty(config.tokens)) {
...
export class googleauth_credentials {
    ...
    async init() {
        if (Util.IsNullEmpty(this.clientid) || Util.IsNullEmpty(this.clientsecret) || Util.IsNullEmpty(this.redirecturi)) return;
        const me = this;
        ...
        if (!Util.IsNullEmpty(this.tokens)) {
...
export class googleauth_request {
    ...
    constructor(public config: Igoogleauth_request) {
        try {
            ...
            if (this.config.ignoretls != null) {
                this.ignoretls = Util.parseBoolean(this.config.ignoretls);
            } else {
                this.ignoretls = false;
            }
            if (this.config.asjson != null) {
                this.asjson = Util.parseBoolean(this.config.asjson);
            } else {
                this.asjson = true;
            }
    ...
    async oninput(msg: any, send: any, done: any) {
        try {
            ...
            if (!Util.IsNullEmpty(msg.method)) this.method = msg.method;
            if (!Util.IsNullEmpty(msg.url)) this.url = msg.url;

            if (Util.IsNullEmpty(this.method)) this.method = msg.method;
            if (Util.IsNullEmpty(this.method)) this.method = "GET";
            if (Util.IsNullEmpty(this.url)) this.url = msg.url;
            if (Util.IsNullEmpty(this.url)) throw new Error("url is mandaotry");
            let url = this.url;
            if (!Util.IsNullUndefinded(this._config) && this._config.authtype == "apikey" && !Util.IsNullEmpty(this._config.apikey)) {
                if (url.indexOf("?") > -1) {
            ...
            if (this.Client != null) {
                ...
                if (!Util.IsNullUndefinded(this._config) && this._config.authtype == "username") {
                    if (!Util.IsNullEmpty(this._config.username)) {
                        options.headers = {};
